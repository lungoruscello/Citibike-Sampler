...
def test_time_normalisation_works():
    ...
    assert normalise_time_range(start, end) == (start, end)  # noqa

    (start_y, start_m), (end_y, end_m) = normalise_time_range("2020-6")
    assert start_y == end_y == 2020
    assert start_m == end_m == 6

    (start_y, start_m), (end_y, end_m) = normalise_time_range("2021", "2021")
    assert start_y == 2021
    assert start_m == 1
    assert end_y == 2021
    assert end_m == 12

    (start_y, start_m), (end_y, end_m) = normalise_time_range("2022-4", "2023-9")
    assert start_y == 2022
    assert start_m == 4
    assert end_y == 2023
    assert end_m == 9
...
def test_download_range_validation_works():
    from citibike_sampler.downloader import _validate_download_range
    ...
    _validate_download_range((LAST_BUNDLED_YEAR, 2), (LAST_BUNDLED_YEAR, 4))  # logs warning, but no error
...
def _test_time_validation_raises(year, month):
    from citibike_sampler.downloader import _validate_download_range  # noqa
