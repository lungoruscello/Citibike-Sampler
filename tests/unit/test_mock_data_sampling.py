...
def test_data_sampling(mocked_download):
    ...
    with switched_cache_dir("sampling_test"):
        ...
        df1 = thinned_dataset(seed=42, **shared_args)
        df2 = thinned_dataset(seed=42, **shared_args)
        df3 = thinned_dataset(seed=23, **shared_args)

    for df in [df1, df2, df3]:
        _assert_valid_sample(df)
...
def test_empty_samples_triggers_warning(mocked_download, caplog):
    ...
    with switched_cache_dir("sampling_test"):
        with caplog.at_level(logging.WARNING):
            df = thinned_dataset(2020, 2020, fraction=1e-20, verbose=False)
            assert df.empty
